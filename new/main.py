# -*- coding: utf-8 -*-
"""
Created on Thu Jul 13 20:46:44 2023
...
temp_val = []
dataset = pd.DataFrame(columns=['title', 'summary', 'sentiment','score','published','count'])
for i in range (0,numberOfPosts):
    entry = NewsFeed.entries[i]
    print ('Processing post ',i+1)
    print ('Post Title :',entry.title)
    print ('Post Summary :',entry.summary)
    print ('Post timestamp :',entry.published)
    ...
    if len(entry.summary) > 0 :
        ...
        compound_val = sentiment_scores(entry.summary)
    temp_val = []
    temp_val.append([entry.title, entry.summary, getCategory(compound_val), compound_val, entry.published, 1])
    temp_df = pd.DataFrame(temp_val, columns=['title', 'summary', 'sentiment','score','published','count'])
